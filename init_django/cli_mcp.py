"""CLI interface for MCPs/agents (non-interactive, arguments/flags,
JSON output). Always keep command compatibility and semantics in sync
with ``cli_user.py``."""
...
@click.option(
    "--venv",
    type=click.Choice(["reuse", "recreate", "skip"]),
    default=None,
)
...
def main(
    ...
    try:
        ...
        if (base / ".git").exists():
            ...
            run("git init")
            run(
                "curl -O https://raw.githubusercontent.com/github/gitignore/"
                "main/"
                "Python.gitignore"
            )
