...
@click.command()
def main():
    """Interactive CLI to bootstrap Django projects
    ...
    if venv.exists():
        venv_choice = click.prompt(
            (
                "🌱 What do you want to do about the virtual "
                "environment (.venv)?\n"
                + "\n".join(
                    [
                        f"{i+1}\u20e3  {opt}"
                        for i, opt in enumerate(venv_choices)  # noqa: E501
                    ]
                )
            ),
    ...
    long_dep = (
        "Install minimal and quality dependencies "
        + "(incl. Django REST Framework)"  # noqa: E501
    )
    dep_choices = [long_dep, "Skip this step"]
    dep_choice = click.prompt(
        (
            "📦 Which dependencies do you want to install?\n"
            + "\n".join(
                [f"{i+1}\u20e3  {opt}" for i, opt in enumerate(dep_choices)]
            )  # noqa: E501
        ),
    ...
    if dep_choice == "1":
        django_version = click.prompt(
            "🔢 Django version to install", default="5.2.3"  # noqa: E501
        )
        try:
            ...
            if major < 3:
                click.echo(
                    "⚠️  Django version too old or invalid. "
                    "Using default 5.2.3."  # noqa: E501
                )
    ...
    if (base / "manage.py").exists():
        ...
        proj_choice = click.prompt(
            (
                ...
                "1️⃣  Create Django project (config)\n"
                "2️⃣  Skip this step\nEnter your choice:"
            ),
            type=click.Choice(["1", "2"]),
            default="1",
            ...
            if settings_dir.exists():
                ...
                if settings_choice == "1":
                    ...
                    for f in ["base.py", "dev.py", "prod.py"]:
                        copyfile(
                            TEMPLATES_DIR / "settings" / f"{f}.tpl",
                            settings_dir / f,
                        )
